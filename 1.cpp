...
*/
void QuickSort(long* c, int s, int e) {
    int indopporn = e;
    for (unsigned int i = 0; i <= e; ++i) {
        if ((c[i] > c[indopporn]) && (i < indopporn)) {
            std::swap(c[i], c[indopporn]);
            indopporn = i;

        }
    }
    int opl = indopporn, opr = indopporn;
    if (opl > 0) {
        QuickSort(c, 0, opl - 1);
...
int main() {
    ...
    unsigned long const N =266; //Варьируемый параметр. Можно смело изменять для получения зависимости времени от N. N <= MAX_N
    ...
    //*****************************Тестирование медленной сортировки******************************************************
    {
        ...
        for (unsigned long i = 0; i < std::min(N, 10ul); ++i)
    ...
    //*****************************Тестирование быстрой сортировки********************************************************
    {
        ...
        std::cout << "First min(" << N << ", 10) elements:\n";
        for (unsigned int i = 0; i < std::min(N, 10ul); ++i)
